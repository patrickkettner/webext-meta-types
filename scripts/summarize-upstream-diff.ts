...
export function formatDiffMarkdown(
  ...
  if (diff.modifiedDeclarations.length > 0) {
    ...
    for (const m of diff.modifiedDeclarations) {
      md += `<details>\n<summary><code>${m.path}</code></summary>\n\n`;
      ...
      if (m.modifiedMembers.length > 0) {
        md += `- **Modified members**: ${m.modifiedMembers.map(x => `\`${x}\``).join(", ")}\n`;
      }
      md += `\n</details>\n`;
    }
    md += `\n`;
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("summarize-upstream-diff.ts")) {
  ...
  if (args.length < 5) {
    console.error("Usage: tsx scripts/summarize-upstream-diff.ts <beforeDts> <afterDts> <pkgName> <fromVer> <toVer> [checkPassed] [checkOutputFile]");
    process.exit(1);
  }
  const [beforePath, afterPath, pkgName, fromVer, toVer, checkPassedArg, checkOutputFile] = args;
